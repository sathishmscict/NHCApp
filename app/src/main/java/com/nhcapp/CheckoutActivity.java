...
/*
    ...
    private void SendOrderDetailsToServer() {
        ...
        String url =  AllKeys.WEBSITE+"InsertOrderMst?clientid="+ userDetails.get(SessionManager.KEY_USERID) +"&orderdate="+ dbhandler.convertEncodedString(dbhandler.getDateTime()) +"&type=Multiple";
    ...
    private void getAllProductDetailsFromServer() {
        ...
        String url_getProducts = AllKeys.WEBSITE + "ViewCartData?type=cartdata&userid=" + userDetails.get(SessionManager.KEY_USERID) + "";
                            ...
                            try {
                                ...
                                for (int i = 0; i < arr.length(); i++) {
                                    ...
                                    if (userDetails.get(SessionManager.KEY_CHECKOUT_TYPE).equals("single"))
                                    {
                                        ...
                                        ProductData prod = new ProductData(c.getString(AllKeys.TAG_CART_PRODUCTID), c.getString(AllKeys.TAG_CATEOGRYNAME),  c.getString(AllKeys.TAG_IMAGE), c.getString(AllKeys.TAG_OFFER_PRICE), c.getString(AllKeys.TAG_OFFER_PRICE), c.getString(AllKeys.TAG_REAL_PRICE), c.getString(AllKeys.TAG_UNIT), c.getString(AllKeys.TAG_PRODUCT_QUANTITY), c.getString(AllKeys.TAG_PRODUCT_NAME), c.getString(AllKeys.TAG_LONGDESCR), c.getString(AllKeys.TAG_PRODUCT_QUANTITY), false);


                                        //ProductData prod = new ProductData(c.getString(AllKeys.TAG_SHOPPINFCOST), c.getString(AllKeys.TAG_PRODUCTID), c.getString(AllKeys.TAG_PRODUCTNAME), c.getString(AllKeys.TAG_DESCRIPTION), c.getString(AllKeys.TAG_PRODUCT_CATEGORYID), AllKeys.RESOURSES + c.getString(AllKeys.TAG_IMAGE_URL), c.getString(AllKeys.TAG_SELLERID), c.getString(AllKeys.TAG_PRICE), c.getString(AllKeys.TAG_OFFER_VALUE), c.getString(AllKeys.TAG_OFFER_TAG), c.getString(AllKeys.TAG_COLOR), c.getString(AllKeys.TAG_WEIGHT), c.getString(AllKeys.TAG_HEIGHT), c.getString(AllKeys.TAG_LENGTH), c.getString(AllKeys.TAG_WIDTH), c.getString(AllKeys.TAG_SIZE), c.getString(AllKeys.TAG_MRP), c.getString(AllKeys.TAG_SHIP_DATE), c.getString(AllKeys.TAG_BRAND), c.getString(AllKeys.TAG_INVENTORY), c.getString(AllKeys.TAG_COMITTED_BY));
                                        totalPriceInRs = totalPriceInRs + Double.parseDouble(c.getString(AllKeys.TAG_OFFER_PRICE));
                                        ...
                                        TOTAL_PRICE = TOTAL_PRICE + c.getInt(AllKeys.TAG_OFFER_PRICE) * c.getInt(AllKeys.TAG_PRODUCT_QUANTITY);
                                        //TOTAL_DELIVERY_CHARGES = TOTAL_DELIVERY_CHARGES + c.getInt(AllKeys.TAG_CATEGORY_SHIPPING_COST_BUYER);
                                        //MLM_DISCOUNT = MLM_DISCOUNT+(c.get);
                    ...
                    try {
                        ...
                        if (qty_prod > 1)
                        {
                         ...
                         //   Log.d(TAG, "URL AddToWishList : " + url_addtowishlist);
                            ...
                            url_addtowishlist = AllKeys.WEBSITE + "UsercartData?type=Updatecart&userid="+ userDetails.get(SessionManager.KEY_USERID) +"&productid="+ pd.getProductid() +"&qty="+ qty +"&date="+ dbhandler.convertEncodedString(dbhandler.getDateTime()) +"&status=1&companyid="+ userDetails.get(SessionManager.KEY_COMPANY_ID) +"";
                          ...
                          /*  final int unitprice = (int) (Double.parseDouble(pd.getPrice()) / qty_prod);
                            final int mrpprice = (int) (Double.parseDouble(pd.getMrp()) / qty_prod);*/
                                           ...
                                           // final double mlmdiscoiunt = Double.parseDouble(pd.getMlmDiscount());
                                            ...
                                            Log.d(TAG, "URL AddToWishList : " + url_addtowishlist);
*/
                                               String url_addtowishlist = AllKeys.WEBSITE + "UsercartData?type=Updatecart&userid="+ userDetails.get(SessionManager.KEY_USERID) +"&productid="+ pd.getProductid() +"&qty="+ qty +"&date="+ dbhandler.convertEncodedString(dbhandler.getDateTime()) +"&status=1&companyid="+ userDetails.get(SessionManager.KEY_COMPANY_ID) +"";
                                                Log.d(TAG, "URL UsercartData : " + url_addtowishlist);
        ...
        private void removeCartDetailsToServer(final String ProductId, final String type, final int position, final ProductData pd) {
            ...
            String  url= AllKeys.WEBSITE + "UsercartData?type=deletecart&userid="+ userDetails.get(SessionManager.KEY_USERID) +"&productid="+ pd.getProductid() +"&qty="+ pd.getQuantity() +"&date="+ dbhandler.convertEncodedString(dbhandler.getDateTime()) +"&status=1&companyid="+ userDetails.get(SessionManager.KEY_COMPANY_ID) +"";
